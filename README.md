...
df = pd.read_excel('Retail Sales Analysis_utf.xlsx')
df.shape
df.head()
...
sns.barplot(x='gender', y='total_sale', data=df.groupby('gender', as_index=False)['total_sale'].sum())
plt.title('Total Sales by Gender')
plt.show()
##### From the above graph, we can see most of the buyers are females

### Total sale by age
df['age_group'] = pd.cut(df['age'], bins=range(10, 80, 10), labels=['10-19','20-29','30-39','40-49','50-59','60-69'])
...
top_customers = df.groupby('customer_id', as_index=False)['total_sale'].sum().sort_values(by='total_sale', ascending=False).head(10)
...
[EDA on Retail Dataset.pdf](https://github.com/user-attachments/files/21342357/EDA.on.Retail.Dataset.pdf)
